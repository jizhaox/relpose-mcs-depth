...
#include "solver_depth_intra_2ac_ka_core_36.h"
#include "solver_depth_intra_2ac_ka_core_44.h"
...
// mex(['-I"' eigen_dir '"'],'-O','solver_depth_intra_2ac_ka.cpp')

using namespace std;
using namespace Eigen;

Eigen::MatrixXcd solve_equation_intra_cam_stew_ka_36(double *input, double* zr, double* zi)
{
    const VectorXd data = Map<const VectorXd>(input, 773); // 773 = 83*8 + 4 + 35*3
//    std::cout << data << std::endl;
    Eigen::MatrixXcd sols = solver_depth_intra_2ac_ka_core_36(data);
...
Eigen::MatrixXcd solve_equation_intra_cam_stew_ka_44(double *input, double* zr, double* zi)
{
    const VectorXd data = Map<const VectorXd>(input, 668); // 668 = 83*8 + 4
//    std::cout << data << std::endl;
    Eigen::MatrixXcd sols = solver_depth_intra_2ac_ka_core_44(data);
    for (Index i = 0; i < sols.size(); i++) {
        zr[i] = sols(i).real();
        zi[i] = sols(i).imag();
    }
...
//        mexErrMsgTxt("the 2nd parameter should be 2 by 2!");
 ...
 //       mexErrMsgTxt("the 3rd parameter should be 2*2*2!");
    ...
    {
        mexErrMsgIdAndTxt("JPL:dp_intra_2ac_ka:notDouble", "Input data must be type double.");
    }
    ...
    if (nrhs >= 7)
    {
        ...
        {
            actype = AC_TYPE::INTRA_CAM_CONSTRAINT_PARTIAL;
        }
    }
    ...
    double *coeffs, *input;
    if (actype == AC_TYPE::INTRA_CAM_CONSTRAINT_PARTIAL)
    {
        plhs[3] = mxCreateDoubleMatrix(3, 44, mxCOMPLEX);
        plhs[4] = mxCreateDoubleMatrix(20, 84, mxREAL);
        input = new double[668]; // 668 = 83*8 + 4
    }
    else
    {
        plhs[3] = mxCreateDoubleMatrix(3, 36, mxCOMPLEX);
        plhs[4] = mxCreateDoubleMatrix(26, 84, mxREAL);
        input = new double[773]; // 773 = 83*8 + 4 + 35*3
    }
    input[664] = 1.0;
    input[665] = 1.0;
    input[666] = 1.0;
    input[667] = -sq_tan_half_theta;
    ...
    if (actype == AC_TYPE::INTRA_CAM_CONSTRAINT_PARTIAL)
    {
        sols = solve_equation_intra_cam_stew_ka_44(input, zr, zi);
    }
    else
    {
        sols = solve_equation_intra_cam_stew_ka_36(input, zr, zi);
    }
